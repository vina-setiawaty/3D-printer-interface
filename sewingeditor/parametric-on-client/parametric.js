// Parametric tactile-graphic page (parametric-on-client.html) -- a fork of
// ../parametric/parametric.js that calls the provider (OpenAI or Anthropic)
// DIRECTLY from the browser with a key the user enters, instead of going
// through /api/generate-parametric. Kept as a fully separate copy (own
// folder, own docs, own localStorage key) rather than a shared branch so
// the original server-proxied page is untouched -- see
// ../docs/llm-api-data-flow.md for how the two relate.
//
...
//
// The model returns { chat, calls[] } (PARAMETRIC_SCHEMA below, copied from
// api/generate-parametric.js). Each call is executed locally by
// parametric-catalog.js against the pinned docs/texture_functions.js --
// editing a parameter re-runs the call list with no API round-trip.
//
...
// saveApiKeyFrom/loadApiKeyInto, callLlmDirect, renderCostTracker).
...
// provider-agnostic -- no discriminated union, no open-ended object,
// nothing a strict json_schema mode can choke on. Mirrors
// api/generate-parametric.js's PARAMETRIC_SCHEMA; keep the two in sync.
...
const PARAMETRIC_SCHEMA_NAME = "parametric_graphic";
const PARAMETRIC_MAX_OUTPUT_TOKENS = 32768;
const PARAMETRIC_THINKING_CAPABLE = true;

const PARAMETRIC_STORAGE_KEY = "parametricOnClientSessionState";
const PROMPT_DOCS = [
...
async function onParametricSend() {
  ...
  const thinking = document.querySelector("#pg-thinking").value === "on" && PARAMETRIC_THINKING_CAPABLE;
  const apiKey = document.querySelector(provider === "openai" ? "#pg-openai-key" : "#pg-anthropic-key").value;
  ...
  if (!apiKey) { messages.innerHTML = `<li>enter your ${escapeHtml(provider)} API key first</li>`; return; }
...
function initParametricEditor() {
  ...
  document.querySelector("#pg-openai-key").addEventListener("change", () => saveApiKeyFrom("openai", "pg-openai-key"));
  document.querySelector("#pg-anthropic-key").addEventListener("change", () => saveApiKeyFrom("anthropic", "pg-anthropic-key"));
