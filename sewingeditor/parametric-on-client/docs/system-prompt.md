...
generation call (made directly from the browser to the chosen provider --
see `parametric.js`'s `callLlmDirect` usage). `parametric.js` builds the
full prompt by concatenating, in order: this file, `catalog.md`,
`path-spec.md`, `hardware.md`, `PARAMETER_CONSTRAINTS.md`. Keep this file
and that builder in sync (and note the change in
`../docs/llm-api-data-flow.md`).

This is a fork of `../parametric/docs/system-prompt.md` for the
`parametric-on-client` page -- keep the two in sync unless they're meant to
diverge.
