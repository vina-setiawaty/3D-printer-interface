// LLM-assisted raw G-code generation, for the "Raw gcode" box
// (#raw-gcode-textarea) run via #run-gcode-btn. Separate endpoint
// (/api/generate-gcode) and separate system prompt from the action editor's
// LLM tab (llm.js) — unlike actions, raw gcode has no __variable
...
let lastAppliedGcodeBoxContent = "";

function buildGcodeSystemPrompt() {
...
function collectGcodeBoxState() {
  return {
    gcode: document.querySelector("#raw-gcode-textarea").value.split("\n").filter(l => l.trim() !== ""),
  };
}

function buildGcodeUserMessage(nlDescription, currentState) {
  return `Current gcode box content (JSON):\n${JSON.stringify(currentState, null, 2)}\n\nInstruction: ${nlDescription}`;
}

function buildGcodeRequestBody(nlDescription, provider, model, currentState) {
  return {
    provider,
    model,
...
function validateGeneratedGcode(result) {
  const warnings = [];
...
function applyResultToGcodeBox(result) {
  const gcodeText = (result.gcode || []).join("\n");
  document.querySelector("#raw-gcode-textarea").value = gcodeText;
  lastAppliedGcodeBoxContent = gcodeText;
}

function resetGcodeLlmPanel() {
  document.querySelector("#gcode-llm-description").value = "";
  document.querySelector("#gcode-llm-messages").innerHTML = "";
  document.querySelector("#gcode-llm-explanation").textContent = "";
  document.querySelector("#gcode-llm-status").textContent = "";
  lastAppliedGcodeBoxContent = "";
}

async function onGcodeGenerateClick() {
  const btn = document.querySelector("#gcode-llm-generate-btn");
  const status = document.querySelector("#gcode-llm-status");
  const messages = document.querySelector("#gcode-llm-messages");
  const explanationEl = document.querySelector("#gcode-llm-explanation");
  const description = document.querySelector("#gcode-llm-description").value.trim();
  const secret = document.querySelector("#gcode-llm-app-secret").value;
  ...
  if (!description) {
    messages.innerHTML = "<li>describe the gcode before generating</li>";
    return;
  }
  if (!secret) {
  ...
  const currentState = collectGcodeBoxState();
  const currentGcode = currentState.gcode.join("\n");
  if (currentGcode && currentGcode !== lastAppliedGcodeBoxContent) {
  ...
  try {
    response = await fetch("/api/generate-gcode", {
      ...
      body: JSON.stringify(buildGcodeRequestBody(description, provider, model, currentState)),
    });
  } catch (e) {
    status.textContent = "";
  ...
  const warnings = validateGeneratedGcode(result);
  applyResultToGcodeBox(result);
...
function initGcodeLlmEditor() {
  document.querySelector("#gcode-llm-generate-btn").addEventListener("click", onGcodeGenerateClick);
  document.querySelector("#gcode-llm-app-secret").addEventListener("change", () => saveAppSecretFrom("gcode-llm-app-secret"));
  document.querySelector("#gcode-llm-provider").addEventListener("change", () => updateModelOptions("gcode-llm-provider", "gcode-llm-model"));
  updateModelOptions("gcode-llm-provider", "gcode-llm-model");
  loadAppSecretInto("gcode-llm-app-secret");
  renderCostTracker();
}
